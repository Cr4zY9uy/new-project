...
function SingUp(props) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url = 'http://139.180.186.20:3003/auth/register';
        ...
        <div className="container w-25">
            <form method='post' onSubmit={handleSubmit}>
                ...
                <div className='d-flex justify-content-center '>
                    <Button type="submit" className="btn btn-success w-100" >Sign up</Button></div>
            </form>
...
const mapDispatchToProps = (dispatch) => {
    return {
        addUser: (user) => {
            dispatch({ type: USER_ACTION.REGISTER, payload: user })
        }
    }
}
